...
class BoingApp extends App {
  ...
  void loadImage(List<String> names, [double pivotX = 0.0, double pivotY = 0.0]) {
    for (var name in names) {
      assets[name] = resources.loadImage("assets/images/$name.png", pivotX: pivotX, pivotY: pivotY);
    }
  }
  ...
  void loadImageAnim(String prefix, int count, [int players = 1, double pivotX = 0.0, double pivotY = 0.0]) {
    if (players == 1) {
      for (int i = 0; i < count; i++) {
        assets["$prefix$i"] = resources.loadImage("assets/images/$prefix$i.png", pivotX: pivotX, pivotY: pivotY);
      }
    } else {
      for (int p = 0; p < players; p++) {
        for (int i = 0; i < count; i++) {
          assets["$prefix$p$i"] = resources.loadImage("assets/images/$prefix$p$i.png", pivotX: pivotX, pivotY: pivotY);
        }
      }
    }
  }
  ...
  void _loadAssets() {
    ...
    soundMap.forEach((name, count) {
      for (int i = 0; i < count; i++) { sounds["$name$i"] = resources.loadSound("assets/sounds/$name$i.ogg"); }
    });
    // dart format on
  }
